...
class GaussNewtonPINNOptimizer:
    ...
    def step(
        ...
        jt_residual = torch.autograd.grad(
        0.5 * loss_before,  # scalar
        vec_req,
        retain_graph=True,
        )[0].reshape(-1)
       ...
       # jacobian = jacobian.reshape(flat_residual.numel(), -1)
        ...
        # recompute residual for the current vec_req to ensure a valid graph
        #r = residual_with_graph(vec_req).reshape(-1)
        #jt_residual = torch.autograd.grad(
        #r,
        #vec_req,
        #grad_outputs=r,   # computes Jᵀ r
        #retain_graph=True
        #)[0].reshape(-1)


        #r = residual_with_graph(vec_req).reshape(-1)
        #jt_residual = torch.autograd.grad(r, vec_req, grad_outputs=r, retain_graph=True)[0].reshape(-1)
        
        #jt_residual = torch.autograd.grad(
        #flat_residual,
        #vec_req,
        #grad_outputs=flat_residual,  # this is r^T J
        #retain_graph=True
        #)[0].reshape(-1)

        diag = torch.ones_like(jt_residual) * self.jtj_diagonal_eps  # crude damping baseline
      ...
      #  def jacobian_vector_product(v: torch.Tensor) -> torch.Tensor:
       ...
       #     return jvp.reshape(-1)
        ...
        def jacobian_vector_product(v: torch.Tensor) -> torch.Tensor:
    # Forward-mode: compute J v
            _, jvp = torch.autograd.functional.jvp(
            residual_with_graph, (vec_req,), (v.view_as(vec_req),),
            create_graph=False
            )
            return jvp.reshape(-1)

        #def jacobian_transpose_vector_product(w: torch.Tensor) -> torch.Tensor:
    # Reverse-mode: compute J^T w
         #   r = residual_with_graph(vec_req).reshape(-1)
          #  grad = torch.autograd.grad(
           # r, vec_req,
           # grad_outputs=w.reshape(r.shape),
           # retain_graph=True
           # )[0]
           # return grad.reshape(-1)

        def jacobian_transpose_vector_product(w: torch.Tensor) -> torch.Tensor:
            w_detached = w.reshape(flat_residual.shape).detach()
            grad = torch.autograd.grad(
            flat_residual,
            vec_req,
            grad_outputs=w_detached,
            retain_graph=True,
            )[0]
            return grad.reshape(-1)
       ...
       #def damped_matvec(v: torch.Tensor, damp: float) -> torch.Tensor:
        ...
        while attempts < self.max_attempts and damping <= self.damping_cap:
            attempts += 1
            delta_direct = None  # no direct solve now
